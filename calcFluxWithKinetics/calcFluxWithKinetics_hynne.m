...
for t = 1:nT% List of reactions
    ...
    if ismember(1,knownKinetics)
        flux(t,1) = params(62)*(params(63)-concMatrix(t,17));
    end

    %Glucose uptake
    if ismember(2,knownKinetics)
    flux(t,2) = (params(1)/params(61))*(concMatrix(t,17)/params(2))/(1 + concMatrix(t,17)/params(2) + ((params(3)*concMatrix(t,17)/params(2) + 1)/(params(3)*concMatrix(t,2)/params(2) + 1)) * (1 + concMatrix(t,2)/params(2) + concMatrix(t,8)/params(4) + (concMatrix(t,2)*concMatrix(t,8))/(params(2)*params(5))))...
        - (params(6)/params(61))*(concMatrix(t,2)/params(2))/(1 + concMatrix(t,2)/params(2) + ((params(3)*concMatrix(t,2)/params(2) + 1)/(params(3)*concMatrix(t,17)/params(2) + 1)) * (1 + concMatrix(t,2)/params(2)) + concMatrix(t,8)/params(4) + (concMatrix(t,2)*concMatrix(t,8))/(params(2)*params(5)));
    end
    ...
    if ismember(13,knownKinetics)
        flux(t,13) = (params(48)/params(61))*(concMatrix(t,4) - concMatrix(t,15));
    end
    
    %Ethanol flow
    if ismember(14,knownKinetics)
        flux(t,14) = params(62)*concMatrix(t,15);
    end
    ...
    if ismember(16,knownKinetics)
        flux(t,16) = (params(54)/params(61))*(concMatrix(t,3) - concMatrix(t,16));
    end
    
    %Glycerol flow
    if ismember(17,knownKinetics)
        flux(t,17) = params(62)*concMatrix(t,16);
    end
    
    %Acetaldehyde out
    if ismember(18,knownKinetics)
        flux(t,18) = (params(55)/params(61))*(concMatrix(t,1) - concMatrix(t,10));
    end
    
    %Acetaldehyde flow
    if ismember(19,knownKinetics)
        flux(t,19) = params(62)*concMatrix(t,10);
    end
    
    %Cyanide-Acetaldehyde flow
    if ismember(20,knownKinetics)
        flux(t,20) = params(56)*concMatrix(t,10)*concMatrix(t,13);
    end
    
    %Cyanide flow
    if ismember(21,knownKinetics)
        flux(t,21) = params(62)*(params(64) - concMatrix(t,13));
    end
